...
You need **Power BI Desktop** for these AppSource assets. Use visual package **1.7.1.0** from this repo (`downloads/ganttChart.pbiviz`). Do not mix versions between the `.pbix` and the uploaded `.pbiviz`.
...
When finished, keep outputs ready for Partner Center upload:

- `DataLundGantt-Sample.pbix` (anywhere convenient)
- `screenshot-01.png` … `screenshot-05.png` (optional count 1–5)
...
- Groups: Discovery / Build / Launch (collapsible; Expand / Collapse in the toolbar when Group is bound)
- Milestones: **Alpha Gate**, **Go-Live** (zero duration)
- **QA** driven by Duration (no End)
- Today line if “Show today line” is on
- Optional: Format → General → **Show time window** for 3 / 6 / 9 / 12 months from today
...
5. Turn on format options worth showcasing:
   ...
   - Optional: Show time window = On (toolbar 3M/6M/12M/All)
...
Groups: Expand / Collapse in the toolbar; collapse rows in the label pane.

Optional time window: Format → General → Show time window (from today).
...
5. Replace `assets/store/screenshot-placeholder-1366x768.png` with real shots (do not upload the placeholder).
...
| `screenshot-04.png` | Format pane open (Bars / General) | Format pane for bars, labels, axis, weekends, time window |
...
Run through these once and fix anything that errors:
...
- [ ] Expand / Collapse groups from the toolbar
- [ ] Resize narrow/wide; scroll label pane if many tasks
- [ ] High contrast (Windows) smoke look

When the `.pbix` and screenshots are ready, continue with [`PUBLISH.md`](./PUBLISH.md) Phase 3 and [`PARTNER_CENTER_LISTING.md`](./PARTNER_CENTER_LISTING.md).
